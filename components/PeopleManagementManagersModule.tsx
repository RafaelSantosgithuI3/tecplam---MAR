...
import { Shield, Plus, Search, User as UserIcon, List, ArrowLeft, CheckCircle, Clock, Save, Download, HandMetal, Scan, X, Upload, Hand, Pencil, Trash2 } from 'lucide-react';
...
export const PeopleManagementManagersModule: React.FC<Props> = ({ onBack, currentUser, hasTabAccess }) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (shiftVal === '1º TURNO') shiftVal = '1º TURNO';
                    else if (shiftVal === '2º TURNO') shiftVal = '2º TURNO';
                    ...
                    if (leaderVal) {
                        ...
                        if (leaderMatch) {
                            ...
                            console.warn(`Líder ${leaderVal} não encontrado para ${matricula}.`);
                        }
                    }

                    payloadEmployees.push({
                        matricula,
                ...
                if (payloadEmployees.length > 0) {
                    await apiFetch('/employees/bulk', {
                        method: 'POST',
                        body: JSON.stringify({ employees: payloadEmployees })
                    });
                    alert(`Importação de ${payloadEmployees.length} colaboradores concluída com sucesso! Os registros existentes foram atualizados.`);
                    loadBaseData();
                } else {
    ...
    const uniqueLeaders = useMemo(() => Array.from(new Set(safeEmployees.map(e => e.superiorId || e.leader).filter(Boolean))).sort(), [employees]);
            ...
            <div className="flex flex-col gap-4 bg-white dark:bg-zinc-900 p-4 rounded-lg border border-slate-200 dark:border-zinc-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 lg:grid-cols-5 xl:grid-cols-5 gap-4 items-end w-full">
                    ...
                    <div className="flex flex-col gap-1 w-full">
                        ...
                        <select value={filterLeader} onChange={e => setFilterLeader(e.target.value)} className="w-full bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl px-4 py-2.5 text-slate-900 dark:text-zinc-100 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500">
                            <option value="ALL">Todos</option>
                            {uniqueLeaders.map(t => {
                                const leaderObj = leaders.find(l => l.matricula === t);
                ...
                <div className="flex flex-wrap gap-2 w-full justify-end mt-2">
                    ...
                    }} className="w-full md:w-auto"><Download size={16} /> Baixar</Button>
                </div>
            </div>
    ...
    const attendanceUniqueLeaders = useMemo(() => {
        const safeEmployees = Array.isArray(employees) ? employees : [];
    ...
    const renderPresenca = () => {
        ...
        const allTeam = employees.filter(e =>
            isActiveEmployee(e) &&
            (attendanceShiftFilter === 'ALL' || e.shift === attendanceShiftFilter) &&
        ...
        const filteredTeam = allTeam.filter(emp => !hasLogForOperationalDay(emp, attDate));
                                ...
                                <div className="flex flex-col xl:flex-row gap-3 w-full">
                                    ...
                                    <div className="flex flex-col gap-1 flex-1">
                                        ...
                                        <select className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl px-4 py-2.5 text-slate-900 dark:text-zinc-100 text-sm w-full focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500" value={attendanceLeaderFilter} onChange={e => setAttendanceLeaderFilter(e.target.value)}>
                                            ...
                                            {attendanceUniqueLeaders.map(leader => (
    ...
    const renderLuvas = () => {
        ...
        const handleUpdateGlove = async (matricula: string, field: 'gloveSize' | 'gloveType' | 'gloveExchanges', value: string | number) => {
            ...
            try {
                const response = await apiFetch('/employees', { method: 'POST', body: JSON.stringify({ ...updatedProfile, fullName: empToUpdate.fullName || empToUpdate.name || currentUser?.name, sector: empToUpdate.sector || "PRODUÇÃO", isEdit: true }) });
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-zinc-800">
                                {processedList.map(s => (
                                    <tr key={s.matricula} className={`hover:bg-slate-50 dark:hover:bg-zinc-800/50 ${s.matricula === selectedLeaderId ? 'bg-indigo-50/30 dark:bg-indigo-900/10' : ''}`}>
                                        ...
                                        <td className="p-4">{s.fullName || s.name || 'Colaborador Sem Nome'} {s.matricula === selectedLeaderId && <span className="text-xs text-indigo-500 font-bold ml-2">(Líder)</span>}</td>
    ...
    const [originalEditMatricula, setOriginalEditMatricula] = useState('');
    ...
    const handleUpdateEmployee = async () => {
        ...
        try {
            await apiFetch('/employees', { method: 'POST', body: JSON.stringify({ ...formData, isEdit: true, originalMatricula: originalEditMatricula !== formData.matricula ? originalEditMatricula : undefined }) });
        ...
        <div className="space-y-4">
                            ...
                            {editFilteredEmployees.map(emp => (
                                <div
                                    ...
                                    onClick={() => {
                                        ...
                                        setOriginalEditMatricula(emp.matricula);
                                        setEditFound(true);
                                    }}
                                    className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-cyan-500 cursor-pointer transition-all"
                                >
            ...
            {editFound && (
                <Card className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <Input label="Matrícula" value={formData.matricula} onChange={e => setFormData(prev => ({ ...prev, matricula: e.target.value }))} disabled={formData.type !== 'TEMPORARIO'} />
                        <div className="flex flex-col gap-2">
                            <label className="text-sm font-medium text-slate-700 dark:text-zinc-300">Foto</label>
                            <input type="file" accept="image/*" onChange={handlePhotoChange} className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-cyan-50 file:text-cyan-700 hover:file:bg-cyan-100 dark:file:bg-zinc-800 dark:file:text-cyan-400" />
                            {formData.photo && <img src={formData.photo} alt="Preview" className="h-20 w-20 object-cover rounded mt-2" />}
